 ...
 * See library/acl.inc for the list of currently supported 
 * Access Control Objects(ACO), which this script will install.  This 
 * script also creates several ARO groups, an "admin" ARO, and 
 * some reasonable ACL entries for the groups.
 *
...
// of these terms by the translation engine. -Rod 
...
if ($gacl->add_object_section('Accounting', 'acct', 10, 0, 'ACO') === FALSE) {
    echo "Unable to create the access controls for OpenEMR.  You have likely already run this script (acl_setup.php) " .
         "successfully.<br>Other possible problems include php-GACL configuration file errors (gacl.ini.php or gacl.class.php).<br>";
    return;
}
// xl('Administration')
...
$gacl->add_object('reports', 'Clinic Reports'     , 'rpt_f' , 10, 0, 'ACO');
// xl('Visits reports')
$gacl->add_object('reports', 'Visit Reports'      , 'rpt_v' , 10, 0, 'ACO');
// xl('Financial reports') _d i.e. dollars
// - note rep and rep_a financial reports are under Accounting, plan to move in next revision - SW
//$gacl->add_object('reports', 'Financial Reports'  , 'rpt_d' , 10, 0, 'ACO');
// xl('Procedure Reports')
$gacl->add_object('reports', 'Procedure Reports'  , 'rpt_p' , 10, 0, 'ACO');
// xl('Insurance reports') 
$gacl->add_object('reports', 'Insurance Reports'  , 'rpt_i' , 10, 0, 'ACO');
// xl('Services Reports')
$gacl->add_object('reports', 'Services'           , 'rpt_s' , 10, 0, 'ACO');
// xl('Blank Forms Reports')
$gacl->add_object('reports', 'Blank Forms Reports', 'rpt_b' , 10, 0, 'ACO');
...
/* Create the Administrator in the above-created "users" section
   and add her to the above-created "admin" group.
   If this script is called by OpenEMR's setup, then will
...
$gacl->add_acl(
    array(
        ...
        'menus'         => array('modle'),
        'reports'       => array('rpt_e', 'rpt_c', 'rpt_f', 'rpt_p', 'rpt_s', 'rpt_v', 'rpt_b')
    ), 
