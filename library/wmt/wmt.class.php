...
class wmtInsurance {
	...
	public function __construct($id = false) {
		...
		if ($this->subscriber_DOB && $this->subscriber_DOB != '0000-00-00' && strtotime($this->subscriber_DOB)) { // strtotime returns FALSE on date error
	...
	public static function getPidInsurance($pid, $type = null) {
		...
		while ($data = sqlFetchArray($results)) {
			...
			if ($data['type'] == 'fp' && !$list[3]) $list[3] = new wmtInsurance($data['id']);
			if ($data['type'] == 'bh' && !$list[4]) $list[4] = new wmtInsurance($data['id']);
		}
	...
	public static function getPidInsDate($pid, $date, $type = null) {
		...
		if(!$date || strtotime($date) === false) // strtotime returns FALSE invalid date
 ...
 *
 */
if (!class_exists('wmtImmunization')) {
	...
	*/
	...
	public function update() {
		...
		foreach ($this as $key => $value) {
			...
			if ($key == 'reviewed_datetime' && $value) $status = 'Reviewed';
			if ($key == 'notified_datetime' && $value) $status = 'Notified';
		}
		...
		// update report records when reviewed/notified
		if ($status) {
			sqlStatement("UPDATE procedure_report SET review_status = ? WHERE procedure_order_id = ?", array($status, $this->order_number));
		}
...
class wmtResult {
	...
	/**
	 * Search and retrieve a list of order results
	 *
	 * @static
	 * @parm string $order_num Order number for the order
	 * @return wmtOrder $object
	 */
	public static function fetchResultList($order_num, $order_seq) {
		if(!$order_num) return false;
		$result_list = array();

		$result = sqlStatement("SELECT procedure_report_id FROM procedure_report WHERE procedure_order_id = ? AND procedure_order_seq = ?",
				array($order_num, $order_seq));
		
		if (!$result) return false;
		while ($report = sqlFetchArray($result)) {
			$result_list[] = new wmtResult($report['procedure_report_id']);
		}
 ...
 * @subpackage Forms
 */
if (!class_exists('wmtSpecimenItem')) {
...
class wmtIssue {
	...
	public static function issueTable($pid, $category, $active, $limit = false) {
		...
		$output = "<table><tr>\n";
		if ($category == 'medical_problem') $output .= "<th class=\"wmtHeader\" style=\"width:60%\">Issue</th>\n";
		if ($category == 'allergy') $output .= "<th class=\"wmtHeader\" style=\"width:60%\">Allergy</th>\n";
		if ($category == 'medication') $output .= "<th class=\"wmtHeader\" style=\"width:60%\">Medication</th>\n";
		if ($category != 'medication') $output .= "<th class=\"wmtHeader\" style=\"width:100px;min-width:100px\">Began</th>\n";
		if ($category == 'medical_problem') $output .= "<th class=\"wmtHeader\" style=\"min-width:100px\">Diagnosis</th>\n";
		...
		$output .= "<th class=\"wmtHeader\" style=\"width:40%\">Comments</th>\n";
		$output .= "<th class=\"wmtHeader\" style=\"width:30px\">Action</th>\n";
		$output .= "</tr>\n";
