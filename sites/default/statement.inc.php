...
function make_statement($stmt)
{
    ...
    if (!empty($_REQUEST['form_email']) && OEGlobalsBag::getInstance()->get('statement_appearance') == "2") {
...
function report_header_2($stmt, $providerID = '1')
{
    ...
    if ($haveLogo === null) {
        $haveLogo = false;
        $practice_logo = OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . "/images/practice_logo.png";
...
// Billing location modified by Daniel Pflieger at Growlingflea Software
    ...
    if (OEGlobalsBag::getInstance()->getBoolean('statement_message_to_patient')) {
        ...
        $out .= "<p>" . nl2br(wordwrap(OEGlobalsBag::getInstance()->getString('statement_msg_text'), 80, "\n", true)) . "</p>";
    ...
    $qr_path = OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . '/images/payment_qr.png';
...
function create_cms_statement($stmt)
{
    ...
    #minimum_amount_to _print
    if ($stmt['amount'] <= (OEGlobalsBag::getInstance()->get('minimum_amount_to_print')) && OEGlobalsBag::getInstance()->getBoolean('use_statement_print_exclusion') && ($_REQUEST['form_category'] != "All")) {
        return "";
    }
    // These are your clinics return address, contact etc.  Edit them.
    // TBD: read this from the facility table
...
{
    global $page_count;
    $png = OEGlobalsBag::getInstance()->get('OE_SITE_DIR') . "/images/" . convert_safe_file_dir_name(OEGlobalsBag::getInstance()->getString('statement_logo'));
    if ($page_count > 1) {
        $pdf->ezNewPage();
    }
