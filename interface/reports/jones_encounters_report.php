    ...
    <tr>
      ...
      <td>
        ...
        <div class="checkbox">
          ...
          <div class="checkbox">
              <label><input type='checkbox' name='form_137' title='<?php echo xla('Pain assess'); ?>'<?php echo ($form_137) ? ' checked' : ''; ?>>
                  <?php  echo xlt('137 patients only'); ?></label>
          </div>
          <div class="checkbox">
              <label><input type='checkbox' name='form_138' title='<?php echo xla('Pain assess'); ?>'<?php echo ($form_138) ? ' checked' : ''; ?>>
                  <?php  echo xlt('138 patients only'); ?></label>
          </div>
          <div class="checkbox">
...
<tbody>
<?php
$mela_dx = array('C43.0', 'C43.10',
    'C43.11', 'C43.12', 'C43.20', 'C43.21', 'C43.22', 'C43.30',
    'C43.31', 'C43.39', 'C43.4', 'C43.51', 'C43.52', 'C43.59',
    'C43.60', 'C43.61', 'C43.62', 'C43.70', 'C43.71', 'C43.72',
    'C43.8', 'C43.9', 'D03.0', 'D03.10', 'D03.11', 'D03.12',
    'D03.20', 'D03.21', 'D03.22', 'D03.30', 'D03.39', 'D03.4',
    'D03.51', 'D03.52', 'D03.59', 'D03.60', 'D03.61', 'D03.62',
    'D03.70', 'D03.71', 'D03.72', 'D03.8', 'D03.9', 'C43.0',
    'C43.10', 'C43.11', 'C43.12', 'C43.20', 'C43.21', 'C43.22',
    'C43.30', 'C43.31', 'C43.39', 'C43.4', 'C43.51', 'C43.52',
    'C43.59', 'C43.60', 'C43.61', 'C43.62', 'C43.70', 'C43.71',
    'C43.72', 'C43.8', 'C43.9', 'D03.0', 'D03.10', 'D03.11',
    'D03.12', 'D03.20', 'D03.21', 'D03.22', 'D03.30', 'D03.39',
    'D03.4', 'D03.51', 'D03.52', 'D03.59', 'D03.60', 'D03.61',
    'D03.62', 'D03.70', 'D03.71', 'D03.72', 'D03.8', 'D03.9',
    'Z85.820');

$cpts_137 = array('99201', '99202', '99203', '99204', '99205',
      '99211', '99212', '99213', '99214', '99215');

$cpts_138 = array('11600', '11601', '11602', '11603', '11604', '11606',
...
if ($res) {
    ...
    while ($row = sqlFetchArray($res)) {
        ...
        $dres = sqlStatement("SELECT code from billing as b WHERE b.code_type = 'ICD10' " .
                            "AND b.pid = ? AND b.encounter = ?", array($patient_id, $patient_enc));
        $match = false;
        $counter = 0;
        $icd10_mela = false;
        ...
        while ($drow = sqlFetchArray($dres)) {
            ...
            //if ($patient_id = 6114) {
             //error_log("counter is $counter and $patient_id has $icd10");
                ...
                //exit;
            //}
            if (in_array("$icd10", $mela_dx)) {
                //error_log("$icd10 is in pieces array ");
                if ($form_138) {
                    if ($icd10 == "Z85.820") {
                        $icd10_mela = false;
                    } else {
                        error_log("$patient_id code $drow[code] is 138 mela");
                        $icd10_mela = true;
                    }
                } else {
                    //$match = true;
                ...
                //error_log("$patient_id code $drow[code] is not mela");
                $icd10_mela = false;
            };
        }

        $mela_row = '';
        $mela_res = sqlStatement("SELECT title, enddate FROM lists WHERE pid = ? AND type = 'medical_problem' AND " .
            "title LIKE '%MELANOMA%'", array($patient_id));
        $mela_row = sqlFetchArray($mela_res);

        if ($mela_row && !$icd10_mela && !$form_138) {
            $icd10 = "Z85.820";
            $icd10_mela = true;
            //echo "$patient_id has $mela_row[title] but not in charges ";
        }
        ...
        $bres = sqlStatement("SELECT code, modifier from billing  WHERE code_type = 'CPT4' " .
            "AND encounter = ?", array($row['encounter']));

        while ($brow = sqlFetchArray($bres)) {
            if ($form_137 && in_array($brow['code'], $cpts_137)) {
                $cpt = $brow['code'];
                $mod = $brow['modifier'];
            } else if ($form_138 && in_array($brow['code'], $cpts_138)) {
                $cpt = $brow['code'];
                $mod = $brow['modifier'];
            } else {
                continue;
                ...
                <tr bgcolor='<?php echo attr($bgcolor); ?>'>
                    ...
                    <td>
                        ...
                        $mres = sqlStatement("SELECT provider from insurance_data as ins WHERE ins.type = 'primary' " .
                                                "AND ins.pid = ?", array($patient_id));
                        $mrow = sqlFetchArray($mres);
                        //var_dump($mrow);
                        $ins_code = $mrow['provider'];
                        $medins = array('4','8','17','43','56','68','99','111');

                        //echo "ins code is . $ins_code";
                        if (in_array("$ins_code",$medins)) {
                            echo "Yes";
                        } else {
                            echo "No ";
                            if ($row['age'] >= 65) {
                    ...
                    <td>
                        <?php
                              echo $cpt;
                              //error_log("some code is $brow[code]");
                              //echo 'some code';
                          ?>
                    </td>
                    <td>
                    ...
                    <td>
                        <?php echo "7010F"; // here's the HCPCS code for measure 137?>
                    </td>
