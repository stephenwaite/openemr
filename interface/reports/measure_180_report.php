...
$output_file = fopen("measure_180_export.csv", "w");

$csv_header = "pid,patient_id,date_of_birth,gender,date_of_service,cpt,icd10,numerator_code,modifier,reason\n";
...
if ($res) {
    ...
    while ($row = sqlFetchArray($res)) {
        ...
        $glu_res = sqlStatement("SELECT title FROM lists WHERE pid = ? AND type = 'medication' AND " .
            "(title LIKE '%SONE%' OR title LIKE '%LONE%') AND enddate = ''", array($pid));
        
        $on_glucocorticoid = false;
        if ($glu_row = sqlFetchArray($glu_res)) {
            $on_glucocorticoid = true;
        }
        
        $numerator_code = '4192F';
        $modifier = '';
        $reason = '';
        
        if ($on_glucocorticoid) {
            $gluco_res = sqlStatement("SELECT * FROM rule_patient_data WHERE pid = ? AND item = 'act_glucocorticoid' " .
                "AND date >= ? AND date <= ?", 
                array($pid, $form_from_date, $form_to_date));
            
            if ($gluco_row = sqlFetchArray($gluco_res)) {
                $result = $gluco_row['result'];
                $reason = $gluco_row['reason'];
                if (stripos($result, "no") !== false || stripos($result, "off") !== false) {
                    $numerator_code = '4192F';
                } else {
                    $numerator_code = '4193F';
                }
            } else {
                $numerator_code = '4194F';
                $modifier = '8P';
            }
        }
        ...
        $csv_line = $pid . "," . $row['pubpid'] . "," . $row['dob'] . "," . 
            ...
            $numerator_code . "," . $modifier . "," . '"' . str_replace('"', '""', $reason) . '"' . "\n";
...
fclose($output_file);
echo "Measure 180 report generated: measure_180_export.csv\n";
