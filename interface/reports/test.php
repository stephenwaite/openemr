...
while ($row = sqlFetchArray($res)) {
    ...
    $hmx[$pid]['fname'] = $row['fname'];
    $hmx[$pid]['lname'] = $row['lname'];
    $hmx[$pid]['dob'] = $row['dob'];
    $hmx[$pid]['sex'] = $row['sex'];
...
echo "<pre>";
echo "pid\tgarno\t\t". str_pad('name', 44) . "\tdob\t\tsex\tdos\t\tcpt\tdx\t" .
    "236\t236";
...
foreach ($hmx as $key=>$value) {
    //var_dump($ite);
    echo "\n", $key, "\t", $value['garno'], "\t", str_pad($value['fname'], 22) . " " . str_pad($value['lname'], 22), "\t",
    $value['dob'], "\t", $value['sex'], "\t",
    $value['dos'], "\t", $value['cpt'], "\t",
    $value['htn'], "\t", $value['236'], "\t",
    $value['374'];
