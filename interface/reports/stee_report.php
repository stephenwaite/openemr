...
//$form_date = "2020-07-09";
$form_to_date   = (isset($_POST['form_to_date'])) ? DateToYYYYMMDD($_POST['form_to_date']) : "";
//$form_to_date = "2020-07-09";


function endInsurance($insrow, $selfpay_charges)
{
    ...
    error_log("self pay charges in endin are $selfpay_charges");
    $charges['selfpay'] += $selfpay_charges;
}
...
<body class="body_top">
    ...
    <form method='post' action='stee_report.php' enctype='multipart/form-data' id='theform' onsubmit='return top.restoreSession()'>
        ...
        while ($erow = sqlFetchArray($eres)) {
            ...
            $code_query = "SELECT fee FROM billing WHERE encounter = ? AND pid = ? " .
                "AND code = 'NP150' AND code_type = 'CPT4'";
            $temp = sqlStatement($code_query, array($erow['encounter'], $erow['pid']));
            while ($res = sqlFetchArray($temp)) {
                //if ($temp_array['code'] == "NP150") {
                    $selfpay_charges += $res['fee'];
                    //echo $selfpay_charges;
                    //continue;
                    //array_push($np, $erow['encounter']);
                    error_log("found charges for pid " . $erow['pid'] . " and encounter " . $erow['encounter'] . " " . $selfpay_charges);
                //}
            }

            endInsurance($erow, $selfpay_charges);
            //var_dump($row);
