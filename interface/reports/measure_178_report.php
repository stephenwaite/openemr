...
$output_file = fopen("measure_178_export.csv", "w");
...
if ($res) {
    ...
    while ($row = sqlFetchArray($res)) {
        ...
        $func_res = sqlStatement("SELECT * FROM rule_patient_data WHERE pid = ? AND item = 'act_rafunc' " .
            "AND date >= ? AND date <= ?", 
            array($pid, $form_from_date, $form_to_date));
        
        $numerator_code = '1170F';
        $modifier = '8P';
        $reason = '';
        
        if ($func_row = sqlFetchArray($func_res)) {
            $modifier = '';
            $reason = $func_row['reason'];
        }
...
fclose($output_file);
echo "Measure 178 report generated: measure_178_export.csv\n";
