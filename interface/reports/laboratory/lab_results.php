...
if ($form_refresh) {
	...
	if ($generic !== FALSE) {
		...
		$query1 .= "fo.id, fo.pid, fo.status AS order_status, fo.order_number, fo.reviewed_id, fo.notified_id, fo.result_abnormal, ";
		...
		if ($form_status == 'f')
			$query1 .= "AND fo.result_abnormal > 0 ";
	}
	...
	if ($quest !== FALSE) {
		...
		$query2 .= "fo.id, fo.pid, fo.status AS order_status, fo.order_number, fo.reviewed_id, fo.notified_id, fo.result_abnormal, ";
		...
		if ($form_status == 'f')
			$query2 .= "AND fo.result_abnormal > 0 ";
	}
	...
	if ($labcorp !== FALSE) {
		...
		$query3 .= "fo.id, fo.pid, fo.status AS order_status, fo.order_number, fo.reviewed_id, fo.notified_id, fo.result_abnormal, ";
		...
		if ($form_status == 'f')
			$query3 .= "AND fo.result_abnormal > 0 ";
	}
...
<body class="body_top">
	...
	<form method='post' name='theform' id='theform'	action='lab_results.php' style="width: 100%">
