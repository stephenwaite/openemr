...
<td colspan='3' valign='top' class='fshead' style='height:" . $lheight * 2 . "pt'>";
          ...
          //  $html .= ":";
            ...
            if ($form_fill) {
                $html .= $patdata['fname'] . ' ' . $patdata['mname'] . ' ' . $patdata['lname'] . '&nbsp;&nbsp;&nbsp;';
               $html .= $patdata['street'] .  '&nbsp;&nbsp;' . $patdata['phone_home'] . "<br />\n";
...
<td colspan='3' valign='top' class='fshead' style='height:${lheight}pt'>";
              ...
              $whichpat = $patdata['pid'];
              //echo $whichpat;
              
              
             
              $apptquery = "SELECT * FROM openemr_postcalendar_events WHERE (pc_eventDate = '$whichdate' AND pc_pid = '$whichpat')";
              $apptresult = sqlQuery($apptquery);
              if (!$apptresult) die ("Database access failed accessing appt: " . mysql_error());

              $eventid = $apptresult['pc_eid'];
              //echo $eventid;
              
              $whichdoc = $apptresult['pc_aid'];
              $docquery = "SELECT * FROM users WHERE id = '$whichdoc'";
              $docresult = sqlQuery($docquery);
              if (!$docresult) die ("Database access failed trying to ascertain which doc: " . mysql_error());
  
            $html .= "<br />";
            $html .= xl('Provider', 'r') . ':' . '&nbsp;&nbsp;' . $docresult['fname'] . '&nbsp;' . $docresult['lname'];
            $html .= "<br />";
                    ...
                    foreach (array('primary', 'secondary', 'tertiary') as $instype) {
                        ...
                        if ($row['provider']) {
                            ...
                            if ($insco_name) {
                                $html .= "&nbsp;$insco_name ";
                                if (($row['copay']) !== '')
                                    $html .= $row['policy_number'] . " Copay $" . $row['copay'] . "&nbsp;&nbsp;&nbsp;" . "$$[ ]  CK[ ]  or  CC[ ] <br />" ;
                                     else $html .= "<br />";
