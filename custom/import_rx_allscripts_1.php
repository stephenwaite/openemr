		...
		switch ($row->getRowIndex ()) {
			case 1:
				if (count($xlCols) != PHPExcel_Cell::columnIndexFromString($objXLS->getActiveSheet()->getHighestColumn())) {
					...
					$errFile = 1;
					break 3; // Stop row iteration
				} // End check for valid nbr of columbs
...
<?php if ($errFile > 0) { ?>
	...
	<BR>
	<ol>
		<li>You specified a file that was not generated in required format.
			Try repeating the import process with a valid file.<BR>Hint: Was the
			file saved in .xls format?</li>
		<li>If you continue to get this error for valid export file,
	...
	<form enctype="multipart/form-data"
		...
		<table>
			<tr>
				<td>Please review the extracted records from last uploaded file (Batch <?php echo $thisBatch; ?>).<BR>
					Then press 'View Exceptions' button to see the list of followup	actions.</td>
		...
		<table>
			...
			?>
			<tr>
				<td><?php echo( (strcasecmp($row['pi_error'],"DUPLICATE")?'':'*').$row['pi_date'] ); ?></td>
	...
	<form enctype="multipart/form-data"
		...
		<table>
			<tr>
				<td>Use 'Browse' button to specify ePrescribe generated Excel file:<br>
				<B>Records were last imported upto <?php echo $last_order; ?>.</B>
				</td>
				<td style='text-align:right'><input type="file" name="file" /></td>
				<td style='text-align:right'><input type="submit" name="Action" value="Upload" /></td>
			</tr>
		</table>
		<?php 	
		...
		?>
			<table>
				<tr>
					<td><B>Please review the following <?php echo sqlNumRows($result); ?> exceptions and reconcile the differences in patient names.</B></td>
					<td style='text-align:right'><input type="submit" name="Action" value="Ignore All" />
				</tr>
				<tr>
					<td>Enter pt number(s) and press 'Reprocess' button for manual reconcilation.</td>
					<td style='text-align:right'><input type="submit" name="Action" value="Reprocess" /></td>
				</tr>
			</table>
			<table>
			<tr>
			...
			<tr>
				...
				<td><?php echo( $row['pi_error'] ); ?></td>
				<td><input type="text" size="5" name="pi[<?php echo( $row['id'] ); ?>]" /></td>
			</tr>
